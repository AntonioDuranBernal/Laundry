		...
		<div class="w-full sm:w-4/5 md:w-4/5 lg:w-7/8 p-4 mb-1">
			<form class="rounded" action="{{route('clientes.search')}}" method="POST">
				...
				<div class="relative">
					...
					<button id="buscarcliente" type="submit" class="text-white absolute right-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="submit">Buscar</button>
				</div>
			</form>
		</div>
		<div class="w-full md:w-1/5 lg:w-1/5 xl:w-1/8 p-4 mb-1">
			<form class="px-2 py-0 rounded" action="{{route('clientes.agregar')}}" method="POST">
				@csrf
